...
-- WORKFLOW PROCEDURES FOUND BELOW

-- Adding an item to a room in the DB
DELIMITER $$
...
BEGIN
    ...
    VALUES (roomName, itemName, itemAmount);
...
-- Updating the total amount of an item in a room
DELIMITER $$

CREATE PROCEDURE UpdateItemAmountInRoom(
    IN itemName VARCHAR(50),
    IN newAmount INT,
    IN roomName VARCHAR(50)
)
BEGIN
    UPDATE RoomItems
    SET ItemQuantity = newAmount
    WHERE RoomName = roomName AND ItemName = itemName;
