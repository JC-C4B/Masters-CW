/* CSC6302 Database Principles
	Unit Three: Basic SQL
    Juan Carlos Cabrera
    ANSWER TO #3 FOUND BELOW
*/
...
-- 3(a) Updating the Yoshi games so the series are mapped correctly
-- Even after disabling Safe Updates I was still getting an error, decided to avoid that path to avoid that error. -- JUAN
UPDATE Game AS g
JOIN (SELECT Id FROM Game WHERE GameName LIKE '%Yoshi%') AS t ON g.Id = t.Id
SET g.Series = 2;

-- Checking if it worked -- JUAN
