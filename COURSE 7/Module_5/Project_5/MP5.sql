/* CSC6302 Database Principles
	Unit Five: Database System Design
    Juan Carlos Cabrera
    Stored procedures for homework can be found below!
*/
...
-- (A) Procedure to select a cookbook from a list of cookbooks -- JUAN
...
-- (B) Procedure to select a recipe from a cookbook -- JUAN
DROP PROCEDURE IF EXISTS GetRecipesFromCookbook;

DELIMITER $$

CREATE PROCEDURE GetRecipesFromCookbook(cookbook_name VARCHAR(200))
BEGIN
...
-- (C) Procedure to find the ingredients needed for a recipe (using our procedure from Project 4) -- JUAN
...
-- (D) Procedure to assign a recipe to a particular day -- JUAN

-- Making a table to store our recipes and what day they're assigned to -- JUAN
CREATE TABLE DailyRecipes (
    RecipeName VARCHAR(100) NOT NULL,
    Day VARCHAR(100) NOT NULL,
    PRIMARY KEY (RecipeName, Day),
    FOREIGN KEY (RecipeName) REFERENCES Recipe (RecipeName) ON UPDATE CASCADE ON DELETE CASCADE
);

-- Creating our procedure to assign recipes to days -- JUAN
DELIMITER $$

CREATE PROCEDURE AssignRecipeToDay(recipe_name VARCHAR(100), day VARCHAR(100))
BEGIN
    INSERT INTO DailyRecipes (RecipeName, Day)
    VALUES (recipe_name, day);
END $$
